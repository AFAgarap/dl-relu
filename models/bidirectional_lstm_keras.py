...
"""Bidirectional LSTM-ReLU class written using Keras/TensorFlow"""
...
class BiLstmRNN:
    def __init__(self, return_summary=False, **kwargs):
        ...
        assert (
            type(kwargs["max_length"]) is int
        ), "Expected data type : int, but {} is {}".format(
            kwargs["max_length"], type(kwargs["max_length"])
        )

        assert "num_classes" in kwargs, "KeyNotFound : {}".format("num_classes")
        assert (
            type(kwargs["num_classes"]) is int
        ), "Expected data type : int, but {} is {}".format(
        ...
        assert (
            type(kwargs["trainable"]) is bool
        ), "Expected data type : bool, but {} is {}".format(
            kwargs["trainable"], type(kwargs["trainable"])
        )

        assert "vocabulary_size" in kwargs, "KeyNotFound : {}".format("vocabulary_size")
        assert (
            type(kwargs["vocabulary_size"]) is int
        ), "Expected data type : int, but {} is {}".format(
            kwargs["vocabulary_size"], type(kwargs["vocabulary_size"])
        )
        ...
        def __build__():
            ...
            model.add(e)
            model.add(
                Bidirectional(LSTM(kwargs["num_neurons"][0], return_sequences=True))
            )
            ...
            model.compile(
                loss=kwargs["loss"], optimizer=kwargs["optimizer"], metrics=["accuracy"]
            )
    ...
    def train(self, **kwargs):
        """Trains the instantiated Bidirectional LSTM-ReLU class
