...
class CNN:
    def __init__(self, return_summary=False, **kwargs):
        ...
        assert (
            type(kwargs["input_shape"]) is tuple
        ), "Expected data type : tuple, but {} is {}".format(
            kwargs["input_shape"], type(kwargs["input_shape"])
        )
        ...
        def __build__():
            ...
            model.add(
                Conv2D(
                    32, (3, 3), activation=activation, input_shape=kwargs["input_shape"]
                )
            )
