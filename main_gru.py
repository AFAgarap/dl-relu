...
"""Implements GRU-RNN class for Sentiment Analysis"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='GRU-RNN for Sentiment Analysis')
    ...
    group.add_argument('-c', '--cell_size', required=True, type=int,
                       help='the number of GRU cells per RNN state')
...
def main(argument_values):
    ...
    features_path = argument_values.dataset_features
    labels_path = argument_values.dataset_labels
    ...
    train_dataset, validation_dataset, test_dataset, num_words = train_test_split(reviews, labels, train_fraction=0.8,
                                                                                  ...
                                                                                  batch_size=argument_values.batch_size)

    model = GRU(alpha=argument_values.learning_rate, batch_size=argument_values.batch_size,
                cell_size=argument_values.cell_size, embed_size=argument_values.embed_size,
                num_classes=NUM_CLASSES, num_layers=argument_values.num_layers,
                penalty_parameter=5,
                sequence_length=SEQUENCE_LENGTH,
                num_words=num_words)
    model.train(epochs=argument_values.epochs, log_path=argument_values.log_path,
                checkpoint_path=argument_values.checkpoint_path, dropout_rate=argument_values.dropout_rate,
                train_features=train_dataset[0], train_labels=train_dataset[1],
                validation_features=validation_dataset[0], validation_labels=validation_dataset[1])
    model.predict(test_features=test_dataset[0], test_labels=test_dataset[1],
                  checkpoint_path=argument_values.checkpoint_path)


if __name__ == '__main__':
    arguments = parse_args()

    main(argument_values=arguments)
